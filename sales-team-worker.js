...
function videoV3LocalDirectorPlan(task, channel) {
  const raw = String(task.outputs?.[channel] || task.outputs?.video || ""),
    lines = raw.split(/\n+/).map((line) => line.replace(/^\s*(?:\d+\s*[–—-]\s*\d+\s*秒|\d+\s*秒)?\s*[：:]?\s*/, "").trim()).filter(Boolean);
  if (lines.length < 7)
    throw Object.assign(new Error("VIDEO_V3_PREFLIGHT_FAILED:沒有文字模型時，腳本至少需要7段明確分鏡"), { status: 409, preflight: { pass: false, issues: ["腳本不足7段"] } });
  const total = Math.max(24, Math.min(35, Number(task.production?.totalSeconds) || 30)),
    middle = (total - 6) / 6,
    durations = [3, middle, middle, middle, middle, middle, middle, 3],
    beats = ["hook", "problem", "product-proof", "process", "contrast", "reaction-twist", "resolution", "cta"],
    types = ["kinetic-title", "presenter", "product-ui", "process-graphic", "comparison", "kinetic-type", "presenter-reaction", "product-cta"],
    effects = ["impact-title", "push-in", "ui-highlight", "step-reveal", "split-screen", "beat-cut", "soft-punch-in", "cta-glow"],
    assets = (Array.isArray(task.assets) ? task.assets : []).filter((item) => /^https:\/\//.test(String(item?.url || ""))),
    source = lines.slice(0, 8);
  while (source.length < 8) source.push(source.at(-1));
  let cursor = 0;
  const scenes = source.map((line, index) => {
    const start = cursor, end = index === 7 ? total : cursor + durations[index];
    cursor = end;
    return { id: `scene-${index + 1}`, start, end, storyBeat: beats[index], purpose: line, visual: index === 2 || index === 7 ? "使用指定的真實產品介面素材" : line, narration: line, subtitle: line, assetType: types[index], assetUrl: index === 2 || index === 7 ? String(assets[index === 2 ? 0 : 1]?.url || assets[0]?.url || "") : "", presenter: index === 1 || index === 6, transition: index ? "beat-cut" : "cold-open", effects: [effects[index]], soundDesign: index === 0 || index === 5 || index === 7 ? ["soft-impact"] : ["subtle-whoosh"], camera: index === 1 || index === 6 ? "medium-shot" : "graphic-layout" };
  });
  return { route: videoV3Route(task), hook: source[0], thesis: String(task.statement || source[1]).slice(0, 1000), twist: source[5], climax: source[6], cta: source[7], scenes };
}
async function buildVideoV3DirectorPlan(env, task, channel) { if (!String(env.ANTHROPIC_KEY || "").trim()) { const plan = videoV3LocalDirectorPlan(task, channel), check = videoV3Preflight(plan, task); if (check.pass) return { ...check, attempts: 0, localFallback: true }; throw Object.assign(new Error(`VIDEO_V3_PREFLIGHT_FAILED:${check.issues.join("；")}`), { status: 409, preflight: { pass: false, issues: check.issues } }); } let issues = []; for (let attempt = 1; attempt <= VIDEO_V3_COST_GUARD.directorPlanAttempts; attempt++) { const out = await claude(env, "你是短影音導演，只輸出合法JSON。", videoV3DirectorPrompt(task, channel, issues), 2600); let plan; try { plan = videoV3Json(out.text); } catch { issues = ["JSON格式錯誤"]; continue; } const check = videoV3Preflight(plan, task); if (check.pass) return { ...check, attempts: attempt }; issues = check.issues; } throw Object.assign(new Error(`VIDEO_V3_PREFLIGHT_FAILED:${issues.join("；")}`), { status: 409, preflight: { pass: false, issues } }); }
