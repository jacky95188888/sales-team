...
async function ai(env, prompt) {
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", { method: "POST", headers: { "Content-Type": "application/json", "x-api-key": env.ANTHROPIC_KEY, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 1800, system: "使用繁體中文、台灣口語。只依提供資料判斷；不得捏造個人經驗、數據或來源。輸出 JSON 時必須是有效 JSON。", messages: [{ role: "user", content: String(prompt || "").slice(0, 20000) }] }) });
...
export default {
  ...
  async fetch(req, env, ctx) {
    const url = new URL(req.url), H = cors(req), isThreads = url.pathname.startsWith("/threads-") || url.pathname === "/oauth/threads/callback"; if (!isThreads) return baseWorker.fetch(req, env, ctx); if (req.method === "OPTIONS") return new Response(null, { headers: H });
    try {
